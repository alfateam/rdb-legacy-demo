...
let createCustomer = ["CREATE TABLE _customer (id UNIQUEIDENTIFIER PRIMARY KEY default NEWID(), name VARCHAR(256), balance NUMERIC(10,2), regdate DATETIME, isActive BIT, picture varbinary(max), document nvarchar(max))"]
let createUser = ["CREATE TABLE _user (id UNIQUEIDENTIFIER PRIMARY KEY default NEWID(), userId VARCHAR(256), password VARCHAR(256), email VARCHAR(256))"]
let createOrder = ["CREATE TABLE _order (id UNIQUEIDENTIFIER PRIMARY KEY default NEWID(), orderNo VARCHAR(256), customerId UNIQUEIDENTIFIER  REFERENCES _customer)"]
let createOrderAuto = ["CREATE TABLE _orderauto (id INTEGER PRIMARY KEY IDENTITY(1,1), orderNo VARCHAR(256), customerId UNIQUEIDENTIFIER  REFERENCES _customer)"]
let createOrderLineAuto = ["CREATE TABLE _orderLineauto (id INTEGER PRIMARY KEY IDENTITY(1,1), orderId INTEGER REFERENCES _orderauto, product VARCHAR(256))"]
let createOrderLine = ["CREATE TABLE _orderLine (id UNIQUEIDENTIFIER PRIMARY KEY default NEWID(), orderId UNIQUEIDENTIFIER REFERENCES _order, product VARCHAR(256))"]
...
let createCompositeOrderLine = ["CREATE TABLE _compositeOrderLine (companyId NUMERIC, orderNo NUMERIC, [lineno] NUMERIC, product VARCHAR(256), PRIMARY KEY (companyId,orderNo, [lineNo]))"];
let createDeliveryAddress = ["CREATE TABLE _deliveryAddress (id UNIQUEIDENTIFIER PRIMARY KEY default NEWID(), orderId UNIQUEIDENTIFIER REFERENCES _order, name VARCHAR(256), street VARCHAR(256), postalCode VARCHAR(256), postalPlace VARCHAR(256), countryCode VARCHAR(256), country VARCHAR(256))"];
...
let insertCustomer1 = "INSERT INTO _customer VALUES ('a0000000-0000-0000-0000-000000000000','George',177,'2003-04-12 04:05:06',0,0xAF03083FCE, '[\"foo\", 1, {\"bar\": true}]')";
let insertCustomer2 = "INSERT INTO _customer VALUES ('b0000000-0000-0000-0000-000000000000','John',3045,'2014-05-11 06:49:40.297',1,0xAF03083FCE,null)";
let insertCustomer3 = "INSERT INTO _customer VALUES ('12345678-0000-0000-0000-000000000000','Yoko',8765,'2012-02-10 07:00:40.297',0,0xAF03083FCE,null)";
let insertCustomer4 = "INSERT INTO _customer VALUES ('87654321-0000-0000-0000-000000000000','Johnny',8123,'2011-03-11 06:00:40.297',1,0xAF03083FCE,null)";
let insertCustomer5 = "INSERT INTO _customer VALUES ('87654399-0000-0000-0000-000000000000','Paul',8125,'2011-04-11 06:00:40.297',1,0xAF03083FCA,null)";
...
let insertSql = [].concat(insertCustomers, insertOrders, insertOrderLines, insertDeliveryAddress, insertUsers);
...
function insert(onSuccess, onFailed) {
    ...
    mssql.open(connectionString, (err, client) => {
        ...
        let sql = createSql.concat(insertSql).join(';')

        // let sql = insertSql.join(';')
        // client.query(insertCustomer1);
        // client.query(insertCustomer2);
        // client.query(insertCustomer3);
        // client.query(insertCustomer4);
        // client.query(insertCustomer5);

        client.query(sql, onInserted);        
